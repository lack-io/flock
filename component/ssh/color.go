...
func Color(msg interface{}, fcolor int, args ... int) string {
	if len(args) == 0 {
		return fmt.Sprintf("\033[%dm%s\033[0m", fcolor, msg)
	} else if len(args) == 1 {
		return fmt.Sprintf("\033[%d;%dm%s\033[0m", fcolor, args[0], msg)
	}
	return fmt.Sprintf("\033[%d;%d;%dm%s\033[0m", fcolor, args[0], args[1], msg)
}
